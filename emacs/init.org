...
** Prettify org-mode headings :org:newpkg:
#+begin_src emacs-lisp
  (elpaca org-bullets		  ; Pretty org!!
    ;; I don't like org-bullets' cycling
    (with-eval-after-load 'org-bullets
      (advice-add #'org-bullets-level-char
		  :override
		  (lambda (level)
		    (let ((bullets '("◉" "○" "♦" "♢" "▪" "▫")))
		      (string-to-char
		       (nth (1- (min level (length bullets))) bullets))))))

    (add-hook 'org-mode-hook #'org-bullets-mode))
#+end_src

** Prettify org-mode items, quotes and source blocks
#+begin_src emacs-lisp
